...
public class NoteManager {
	...
	public static Note getNote(Activity activity, Uri uri) {
		...
		ContentResolver cr = activity.getContentResolver();
		Cursor cursor = cr.query(uri, FULL_PROJECTION, null, null, null);
	...
	public static Uri putNote(Activity activity, Note note) {
		...
		// The note identifier is the guid
		ContentResolver cr = activity.getContentResolver();
		Cursor existingCursor = cr.query(notes,
                LIST_PROJECTION,
                Note.GUID + "= ?",
                whereArgs,
                null);
		...
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
			plainContent = XmlUtils.escape(Html.fromHtml(title + "\n" + xmlContent, Html.FROM_HTML_MODE_LEGACY).toString());
		} else {
			plainContent = XmlUtils.escape(Html.fromHtml(title + "\n" + xmlContent).toString());
		}
	...
	public static void deleteNote(Activity activity, String guid)
	{
		...
		if (note != null) deleteNote(activity, note);
	}
	...
	public static Cursor getAllNotes(Activity activity, Boolean includeNotebookTemplates) {
		...
		return activity.getContentResolver().query(notes, LIST_PROJECTION, where, null, sortOrder);
	}
	...
	public static Note[] getAllNotesAsNotes(Activity activity, boolean includeNotebookTemplates) {
		...
		Cursor cursor = activity.getContentResolver().query(uri, FULL_PROJECTION, where, null, orderBy);
		if (cursor == null || cursor.getCount() == 0) {
			if (cursor != null) cursor.close();
	...
	public static Cursor getTitles(Activity activity) {
		...
		return activity.getContentResolver().query(Tomdroid.CONTENT_URI, TITLE_PROJECTION, where, null, null);
	}
	...
	public static Cursor getGuids(Activity activity) {
		...
		return activity.getContentResolver().query(Tomdroid.CONTENT_URI, GUID_PROJECTION, null, null, null);
	}
	...
	public static int getNoteId(Activity activity, String title) {
		...
		Cursor cursor = activity.getContentResolver().query(Tomdroid.CONTENT_URI, ID_PROJECTION, "UPPER("+Note.TITLE+")=?", whereArgs, null);
	...
	public static int getNoteIdByGUID(Activity activity, String guid) {
		...
		Cursor cursor = activity.getContentResolver().query(Tomdroid.CONTENT_URI, ID_PROJECTION, Note.GUID+"=?", whereArgs, null);
	...
	public static Cursor getNewNotes(Activity activity) {
		Cursor cursor = activity.getContentResolver().query(Tomdroid.CONTENT_URI, DATE_PROJECTION, "strftime('%s', "+Note.MODIFIED_DATE+") > strftime('%s', '"+Preferences.getString(Preferences.Key.LATEST_SYNC_DATE)+"')", null, null);
